...
<<<<<<< HEAD
  ...
  try {
    const [jobsData, awData, regionData] = await Promise.all([
      API.getJobs(),
    ...
    const regionName = jobsData.regionName || getRegionName(STATE.player.regionId);
    const aw         = awData;
    const p          = STATE.player;
    const region     = regionData.region || null;
    const regenPerMin = region ? (region.medicine / 10).toFixed(1) : '?';
...
<<<<<<< HEAD
        ...
        <div style="background:var(--bg-card);border:1px solid var(--border);border-radius:var(--radius-lg);padding:12px 14px;margin-bottom:12px">
          <div style="display:flex;align-items:center;justify-content:space-between">
            <div>
              <div style="font-family:var(--font-display);font-size:15px;font-weight:700">📍 ${regionName}</div>
              <div style="font-family:var(--font-mono);font-size:10px;color:var(--text-secondary);margin-top:2px">
                Tu región actual · ⚡ +${regenPerMin}/min
              </div>
            </div>
            <div style="text-align:right">
              ${hasResidency
                ? '<span class="badge badge-success">🏠 RESIDENTE</span>'
                : myResidency?.status === 'pending'
                  ? '<span class="badge badge-warning">⏳ PENDIENTE</span>'
                  : `<button class="btn-ghost btn-sm" onclick="requestResidency()">📋 Solicitar residencia</button>`}
            </div>
          </div>
          ${region && region.hasState
            ? `<div style="margin-top:6px;font-family:var(--font-mono);font-size:10px;color:var(--text-dim)">
                🏛️ Estado: <span style="color:${region.stateColor||'var(--accent)'}">${region.stateShield} ${region.stateName}</span>
               </div>`
        ...
        <div id="jobs-list">
          ${jobs.length === 0
            ? `<div class="card"><div class="empty">
                No hay fábricas disponibles en ${regionName}.<br>
...
async function requestResidency() {
  try {
