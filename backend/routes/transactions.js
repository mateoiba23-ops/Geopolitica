...
router.get('/history', authMiddleware, (req, res) => {
  ...
  let txs = db.getPlayerTransactions(player.id, 500);
...
router.get('/summary', authMiddleware, (req, res) => {
  const player = req.player;
  const txs    = db.getPlayerTransactions(player.id, 500);
  const now    = Date.now();
  const day    = 86400000;
