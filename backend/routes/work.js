...
// Solo muestra fábricas en la región actual del jugador
router.get('/available', authMiddleware, (req, res) => {
  const player          = req.player;
  const regionFactories = db.getFactoriesByRegion(player.regionId);
  const region          = db.getRegion(player.regionId);

  const jobs = regionFactories
  ...
  res.json({ jobs, regionName: region ? region.name : '' });
});
...
router.post('/work', authMiddleware, (req, res) => {
  ...
  // Verificar que la fábrica está en la región del jugador
  if (factory.regionId !== player.regionId) {
    return res.status(403).json({
      error: `Solo puedes trabajar en fábricas de tu región (${db.getRegion(player.regionId)?.name || player.regionId})`
    });
  }
  ...
  // ── 5b. XP de habilidades por trabajo (lento, de persistencia) ───────────
  const skillXpGain    = calcWorkSkillXpGain(workUnits);
  const skillLvlMsgs   = [];
  const eduMsgs  = addSkillXp(player, 'education', skillXpGain.education);
  const endMsgs  = addSkillXp(player, 'endurance', skillXpGain.endurance);
  skillLvlMsgs.push(...eduMsgs, ...endMsgs);
  ...
  res.json({
    ...
    result: {
      ...
      skillXpGain,
      levelMessages:    [...levelMsgs, ...skillLvlMsgs],
